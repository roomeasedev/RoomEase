...
## Design Pattern:
In devoloping RoomEase we used the following design patterns and principles:
...
**Null Object Behavioral Pattern:** When a user is logged in into RoomEase, their user_id is stored locally. The user is later prompted to either join or create a group, which generates and stores a group_is locally as well. In case of the absence of either of these ID's, the code treats it transparently and recovers. The snip of code that does this can be found in `www/js/controller.js` starting line 12: 
```
